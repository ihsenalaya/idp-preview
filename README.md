...
| Microcks | `microcks` | latest | API contract testing — OPEN_API_SCHEMA runner |
| kagent | `kagent-system` | latest | AI agent for automated failure diagnosis |
...
└──────────────────────────┬──────────────────────────────────────│───┘
  ...
  PR comment ✅    kagent triggered 🤖            │             │
         ...
         │  Inspects:               │             │             │
         │  - Preview CR status     │      ┌──────┘             │
         │  - Pod/job logs          │      │  Microcks          │
         │  - Microcks job output   │◄─────│  (in-cluster)      │
         │  - Events                │      │  OPEN_API_SCHEMA   │
         └──────────┬───────────────┘      │  runner            │
                    │                      └────────────────────┘
                    ▼
...
- Is triggered automatically when any test suite (Microcks, regression, E2E)
  fails.
...
kagent is purely observational — it reads, analyses, and reports. The
operator remains the authoritative controller for all cluster state.
...
**Run the OpenAPI validator locally:**
...
**Run the Microcks contract test manually (requires running backend):**
...
**Apply the Microcks Job inside a preview namespace:**
...
# Apply the troubleshooter resources
kubectl apply -f k8s/kagent/namespace.yaml
kubectl apply -f k8s/kagent/rbac-readonly.yaml
# Copy and fill in the example files:
cp k8s/kagent/modelconfig-azure-openai.example.yaml k8s/kagent/modelconfig-azure-openai.yaml
# edit: fill in endpoint, deploymentName, create the api-key Secret
kubectl apply -f k8s/kagent/modelconfig-azure-openai.yaml
kubectl apply -f k8s/kagent/remote-mcp-server.example.yaml
kubectl apply -f k8s/kagent/preview-troubleshooter-agent.yaml
```

### Example kagent troubleshooting output
...
- Expected: HTTP 201 with field `id` (integer)
- Actual: HTTP 201 with field `order_id` (integer)
- Log excerpt:
  FAIL POST /api/orders — field 'id' missing (found 'order_id')

**Likely cause:**
The backend renamed the response field from `id` to `order_id` but
`api/openapi.yaml` was not updated. This is a breaking change.

**Suggested fix:**
Restore `id` in app.py line 293 or update api/openapi.yaml OrderResponse.
...
|--------|-----------|------|---------|
| `microcks-credentials` | `preview-pr-<N>` | `client_id`, `client_secret` | Microcks OAuth2 (optional — omit for open Microcks) |
| `azure-openai-credentials` | `kagent-system` | `api-key` | Model provider API key for kagent |

```bash
# Microcks credentials (per preview namespace — created by operator)
kubectl create secret generic microcks-credentials \
  --namespace preview-pr-42 \
  --from-literal=client_id=<CLIENT_ID> \
  --from-literal=client_secret=<CLIENT_SECRET>

# kagent model API key
...
| Limitation | Detail |
|------------|--------|
| Microcks must be pre-installed | Not installed by the Preview Operator chart — deploy separately |
| OpenAPI spec must be imported in Microcks | Upload `api/openapi.yaml` once via UI or API before first use |
| kagent is read-only | Cannot auto-fix failures — diagnosis only |
| kagent requires a model provider | Azure OpenAI, OpenAI, or any OpenAI-compatible API |
| Microcks authentication is optional | Works with anonymous Microcks for local Kind demos |
| Contract test timeout | Default 30 s per test run — increase `TEST_TIMEOUT_MS` for slow environments |
